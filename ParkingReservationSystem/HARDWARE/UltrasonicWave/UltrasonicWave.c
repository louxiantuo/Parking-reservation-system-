...
void EXTI9_5_IRQHandler(void)
{
	...
	{/*
		...
		{
			num[0]=0;
			if(mark[0]>7)printf("1³µÎ»ÎŞ³µ\t");
			else printf("1³µÎ»ÓĞ³µ\t");
			mark[0]=0;
	}*/
  ...
  if(EXTI_GetITStatus(EXTI_Line12) != RESET)
	{
			...
			TIM_Cmd(TIM3, ENABLE); 		//¿ªÆô
			while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_12));		//µÈ´ıµÍµçÆ½
			TIM_Cmd(TIM3, DISABLE);			                                 //¶¨Ê±Æ÷3Ê§ÄÜ
			UltrasonicWave_Distance=TIM_GetCounter(TIM3)*2*5*34/200.0;									 //¼ÆËã¾àÀë&&UltrasonicWave_Distance<150
		if(UltrasonicWave_Distance>0)
		{
		/*	num[1]++;
			...
			{
				num[1]=0;
				if(mark[1]>7)printf("2³µÎ»ÎŞ³µ\n");
				else printf("2³µÎ»ÓĞ³µ\n");
				mark[1]=0;
			}*/
...
void EXTI1_IRQHandler(void)
{
     ...
     if(EXTI_GetITStatus(EXTI_Line1) != RESET)
	{
			...
			TIM_Cmd(TIM4, ENABLE); 		//¿ªÆô
			while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_1));		//µÈ´ıµÍµçÆ½
			TIM_Cmd(TIM4, DISABLE);					//¶¨Ê±Æ÷3Ê§ÄÜ
			UltrasonicWave_Distance=TIM_GetCounter(TIM4)*2*5*34/200.0;									 //¼ÆËã¾àÀë&&UltrasonicWave_Distance<150
	if(UltrasonicWave_Distance>0)
	{
